...
function displayComposition(composition) {
    ...
    if (composition.markdown_content) {
        // 如果有专门的markdown字段
        markdownContent.innerHTML = marked.parse(composition.markdown_content);
    } else if (composition.workflow) {
        // 从现有字段构建markdown
        const markdown = buildMarkdownFromComposition(composition);
        markdownContent.innerHTML = marked.parse(markdown);
    } else {
        // 显示原始JSON
        markdownContent.innerHTML = `<pre><code>${JSON.stringify(composition, null, 2)}</code></pre>`;
    }
