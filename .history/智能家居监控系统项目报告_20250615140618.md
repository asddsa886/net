...
```mermaid
flowchart LR
    A[start_continuous_collection<br/>启动数据采集] --> B[_simulate_sensor_value<br/>传感器数据模拟]
    B --> C[_validate_data_quality<br/>数据质量检查]
    C --> D{数据是否正常?}
    D -->|是| E[create_observation<br/>生成观测记录]
    D -->|否| F[_mark_anomaly<br/>标记异常数据]
    E --> G[generate_semantic_event<br/>语义事件生成]
    F --> G
    G --> H[_notify_subscribers<br/>通知订阅者]
    H --> I[_save_data<br/>数据存储]
    I --> J[继续采集循环]
    J --> B
