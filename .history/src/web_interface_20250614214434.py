    ...
    <div class="container">
        ...
        <div class="card service-composition">
            ...
            <div id="compositionResult" class="hidden">
                <h4>组合结果：</h4>
                <div id="compositionData" class="composition-output-container"></div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        async function createComposition() {
            ...
            try {
                ...
                if (composition.error) {
                    ...
                    // 美化显示组合结果
                    displayFormattedComposition(composition);
        ...
        function escapeHtml(unsafe) {
            if (typeof unsafe !== 'string') {
                // Ensure non-strings are returned as is, or stringified if appropriate
                if (unsafe === null || unsafe === undefined) return '';
                if (typeof unsafe === 'object') return JSON.stringify(unsafe); // Or handle appropriately
        ...
        function renderSimpleMarkdown(markdownText) {
            ...
            let html = '';
            // Use actual newline character for splitting, as LLM responses typically use \n
            ...
            for (const line of lines) {
                if (line.startsWith('```')) {
                    if (inList) { // Close list if open
                        html += '</ul>';
                        inList = false;
                    }
                    if (inCodeBlock) { // 结束代码块
                        // Ensure codeBlockContent is properly escaped before adding to HTML
                        html += `<pre class="code-block language-${escapeHtml(codeBlockLang)}"><code class="language-${escapeHtml(codeBlockLang)}">${escapeHtml(codeBlockContent.trimEnd())}</code></pre>`;
                        inCodeBlock = false;
                        codeBlockContent = '';
                        codeBlockLang = '';
                    } else { // 开始代码块
                ...
                if (inCodeBlock) {
                    codeBlockContent += line + '\n'; // Append with actual newline for pre formatting
                    continue;
                }
                ...
                if (line.startsWith('### ')) {
                    ...
                    // Regex for *italic* text (simple version, might conflict if used with bold)
            ...
            if (inCodeBlock) { // Ensure final code block is closed if file ends mid-block
                 html += `<pre class="code-block language-${escapeHtml(codeBlockLang)}"><code class="language-${escapeHtml(codeBlockLang)}">${escapeHtml(codeBlockContent.trimEnd())}</code></pre>`;
            }
            return html;
        }
        ...
        // 显示格式化的服务组合结果
                    ...
                    // 简单列表处理，不处理嵌套
                    if (!html.endsWith('</ul>')) { // 开始新列表
                         html = html.replace(/<p>$/, ''); // 移除可能的前一个空p标签
                         if (html.endsWith('</p>')) html = html.substring(0, html.length - 4);
                         html += '<ul>';
                    }
                    html += `<li>${escapeHtml(line.substring(2))}</li>`;
                } else if (line.trim() === '') { // 空行
                    if (html.endsWith('</ul>')) { // 结束列表
                        html += '</ul>';
                    }
                    html += '<br>'; // 或者 <p>&nbsp;</p>
                }
        ...
        // 显示格式化的服务组合结果
        function displayFormattedComposition(composition) {
            const resultDiv = document.getElementById('compositionData');
            resultDiv.innerHTML = ''; // 清空之前的内容

            if (!composition || typeof composition !== 'object') {
                resultDiv.innerHTML = '<p>无法显示组合结果：数据格式错误。</p>';
                return;
            }

            let html = '<div class="composition-details">';

            // 基本信息
            html += `<h5>基本信息</h5>`;
            html += `<p><strong>组合ID:</strong> ${escapeHtml(composition.id)}</p>`;
            html += `<p><strong>创建时间:</strong> ${escapeHtml(new Date(composition.created_at).toLocaleString())}</p>`;
            html += `<p><strong>状态:</strong> <span class="status-${escapeHtml(composition.status)}">${escapeHtml(composition.status)}</span></p>`;

            // 验证结果
            if (composition.validation_results) {
                html += `<h5>验证结果</h5>`;
                const vr = composition.validation_results;
                html += `<p><strong>是否有效:</strong> ${vr.is_valid ? '✔️ 是' : '❌ 否'}</p>`;
                if (vr.errors && vr.errors.length > 0) {
                    html += `<p><strong>错误:</strong></p><ul>`;
                    vr.errors.forEach(err => html += `<li>${escapeHtml(err)}</li>`);
                    html += `</ul>`;
                }
                if (vr.warnings && vr.warnings.length > 0) {
                    html += `<p><strong>警告:</strong></p><ul>`;
                    vr.warnings.forEach(warn => html += `<li>${escapeHtml(warn)}</li>`);
                    html += `</ul>`;
                }
                if (vr.suggestions && vr.suggestions.length > 0) {
                    html += `<p><strong>建议:</strong></p><ul>`;
                    vr.suggestions.forEach(sug => html += `<li>${escapeHtml(sug)}</li>`);
                    html += `</ul>`;
                }
            }
            ...
            if (composition.composition_data) {
                const cd = composition.composition_data;
                html += `<h5>服务详情</h5>`;
                if (cd.services && cd.services.length > 0) {
                    html += `<p><strong>包含的服务 (${cd.services.length}):</strong></p>`;
                    html += `<ul class="service-list">`;
                    cd.services.forEach(service => {
                        html += `<li>
                            <strong>${escapeHtml(service.service_name)} (ID: ${escapeHtml(service.service_id)})</strong>
                            <p>角色: ${escapeHtml(service.role)}</p>
                            <p>优先级: ${'🔥'.repeat(parseInt(service.priority) || 1)} (${escapeHtml(service.priority)})</p>
                            <p>输入: ${escapeHtml(service.inputs.join(', ') || '无')}</p>
                            <p>输出: ${escapeHtml(service.outputs.join(', ') || '无')}</p>
                            <p>依赖: ${escapeHtml(service.dependencies.join(', ') || '无')}</p>
                        </li>`;
                    });
                    html += `</ul>`;
                }
                if (cd.execution_flow) {
                    html += `<p><strong>执行流程:</strong> ${escapeHtml(cd.execution_flow)}</p>`;
                }
                if (cd.data_flow) {
                    html += `<p><strong>数据流:</strong> ${escapeHtml(cd.data_flow)}</p>`;
                }
            }

            // 添加LLM响应的折叠部分 (Markdown渲染)
            if (composition.llm_response) {
                html += `
                    <details>
                        <summary>查看AI响应详情 (Markdown渲染)</summary>
                        <div class="markdown-rendered-response">
                            ${renderSimpleMarkdown(composition.llm_response)}
                        </div>
                    </details>
                `;
            }
            ...
            html += `
                <details>
                    ...
                    <pre style="white-space: pre-wrap; word-wrap: break-word; background: #f0f0f0; padding: 10px; border-radius: 5px;">${escapeHtml(JSON.stringify(composition, null, 2))}</pre>
                </details>
            `;

            html += `</div>`; // close composition-details
