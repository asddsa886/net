    ...
    def _setup_routes(self):
        ...
        def view_markdown(composition_id):
            ...
            try:
                # 尝试从组合历史中获取指定ID的组合
                history = self.llm_composer.get_composition_history()
                composition = next((c for c in history if c.get('id') == composition_id), None)
                
                if composition:
                    # 从历史记录中获取内容
                    raw_markdown_content = composition.get('llm_response', '')
                else:
                    # 如果历史记录中没有，可能是直接的字符串内容
                    # 这里我们可以从 session 或其他方式获取内容
                    # 为了简化，我们返回一个提示
                    raw_markdown_content = "# 服务组合结果\n\n抱歉，无法找到完整的组合结果。请重新生成服务组合。"
...
<body>
    ...
    <script>
        ...
        function renderSimpleMarkdown(markdownText) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (trimmedLine.startsWith('```')) {
                    ...
                    if (inCodeBlock) {
                        html += `<pre class="code-block language-${escapeHtml(codeBlockLang)}"><code>${escapeHtml(codeBlockContent.trimEnd())}</code></pre>`;
                        inCodeBlock = false;
                        codeBlockContent = '';
                        codeBlockLang = '';
                    } else {
                ...
                if (trimmedLine.startsWith('##### ')) {
                    if (inList) { html += '</ul>'; inList = false; }
                    if (inOrderedList) { html += '</ol>'; inOrderedList = false; }
                    html += `<h5>${escapeHtml(trimmedLine.substring(6))}</h5>`;
                } else if (trimmedLine.startsWith('#### ')) {
                    if (inList) { html += '</ul>'; inList = false; }
                    if (inOrderedList) { html += '</ol>'; inOrderedList = false; }
                    html += `<h4>${escapeHtml(trimmedLine.substring(5))}</h4>`;
                } else if (trimmedLine.startsWith('### ')) {
                    if (inList) { html += '</ul>'; inList = false; }
                    if (inOrderedList) { html += '</ol>'; inOrderedList = false; }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const rawMarkdown = {{ raw_markdown_content_json | safe }};
            ...
            outputDiv.innerHTML = renderSimpleMarkdown(rawMarkdown);
                ...
                """, raw_markdown_content_json=json.dumps(raw_markdown_content))
            except Exception as e:
...
<body>
    <div class="container">
        ...
        <div class="card service-composition">
            ...
            <div id="compositionResult" class="hidden">
                ...
                <pre id="compositionData" style="max-height: 300px; overflow-y: auto;"></pre>
            </div>
        </div>
    </div>
    ...
    <script>
            ...
            try {
                ...
                if (composition.error) {
                    ...
                    if (typeof composition === 'string') {
                        // 如果返回的是字符串，直接使用
                        markdownContent = composition;
                        compositionId = Date.now().toString(); // 生成临时ID
                    } else if (composition && typeof composition === 'object') {
                        // 如果返回的是对象
                        compositionId = composition.id || Date.now().toString();
                        markdownContent = composition.llm_response || composition.response || JSON.stringify(composition, null, 2);
                    }
                    
                    // 将内容临时存储到 localStorage (sessionStorage 在新窗口中无法访问)
                    if (markdownContent) {
                        localStorage.setItem(`composition_${compositionId}`, markdownContent);
                        // 设置过期时间（1小时后自动清理）
                        localStorage.setItem(`composition_${compositionId}_expires`, (Date.now() + 3600000).toString());
                    }
                    
                    // 更新"查看完整报告"按钮，使用 JavaScript 方式打开新页面
                    const viewBtn = document.getElementById('viewFullBtn');
                    viewBtn.onclick = function(e) {
                        e.preventDefault();
                        openMarkdownViewer(compositionId, markdownContent);
                    };
                    
                    // 显示组合结果（简化版本）
                    if (typeof composition === 'string') {
                        // 如果是字符串，显示前500字符
                        document.getElementById('compositionData').textContent = 
                            composition.length > 500 ? composition.substring(0, 500) + '...\n\n点击上方"查看完整报告"按钮查看完整内容' : composition;
                    } else {
                        document.getElementById('compositionData').textContent = 
        ...
        // 打开 Markdown 查看器
        function openMarkdownViewer(compositionId, markdownContent) {
            // 创建一个新的窗口来显示 Markdown 内容
...
<body>
    ...
    <button onclick="window.close()" class="back-btn">← 关闭窗口</button>
        ...
        function renderSimpleMarkdown(markdownText) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (trimmedLine.startsWith('\`\`\`')) {
                    ...
                    if (inOrderedList) { html += '</ol>'; inOrderedList = false; }
                    if (inCodeBlock) {
                        html += '<pre class="code-block"><code>' + escapeHtml(codeBlockContent.trimEnd()) + '</code></pre>';
                        inCodeBlock = false;
                        codeBlockContent = '';
                        codeBlockLang = '';
                    } else {
