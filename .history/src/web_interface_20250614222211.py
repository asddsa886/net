...
class WebInterface:
    ...
    def _setup_routes(self):
        ...
        @self.app.route('/view_markdown/<composition_id>')
        def view_markdown(composition_id):
            """查看完整Markdown页面"""
            try:
                # 从组合历史中获取指定ID的组合
                history = self.llm_composer.get_composition_history()
                composition = next((c for c in history if c['id'] == composition_id), None)
                
                if not composition:
                    return "未找到指定的组合结果", 404
                
                # 提取Markdown内容
                raw_markdown_content = composition.get('llm_response', '')
                
                # 返回专门的Markdown查看页面
                # The renderSimpleMarkdown and escapeHtml functions need to be defined in this template's script
...
<head>
    ...
    <style>
        ...
        pre, .code-block { /* Added .code-block for consistency */
            background: #f6f8fa;
            padding: 16px;
            border-radius: 6px;
            overflow-x: auto;
        ...
        code {
            font-family: 'Consolas', 'Courier New', monospace; /* Added Courier New as fallback */
            font-size: 14px;
        }
        ...
        .markdown-body h2 { font-size: 1.75em; margin-bottom: 0.5em; }
        .markdown-body h3 { font-size: 1.5em; margin-bottom: 0.5em; }
        .markdown-body p { margin-bottom: 1em; }
        .markdown-body ul { margin-bottom: 1em; padding-left: 20px; }
    ...
    <script>
        ...
        function escapeHtml(unsafe) {
            if (typeof unsafe !== 'string') {
                ...
                if (typeof unsafe === 'object') return JSON.stringify(unsafe);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const rawMarkdown = {{ raw_markdown_content_json | safe }}; // Pass raw markdown as a JS string
            const outputDiv = document.getElementById('markdownOutput');
            outputDiv.innerHTML = renderSimpleMarkdown(rawMarkdown);
        });
    </script>
</body>
</html>
                """, raw_markdown_content_json=json.dumps(raw_markdown_content)) # Pass as JSON string to handle special chars
            except Exception as e:
                return f"显示Markdown内容时出错: {str(e)}", 500
        ...
        async function createComposition() {
            ...
            try {
                ...
                if (composition.error) {
                    ...
                    // 更新"在新页面查看"按钮的链接
                    document.getElementById('viewFullBtn').href = `/view_markdown/${composition.id}`;
                    
                    // 显示组合结果
                    const compositionData = document.getElementById('compositionData');
                    let html = '<div class="composition-details">';
                    
                    // 基本信息
                    html += `<h5>基本信息</h5>`;
                    html += `<p><strong>组合ID:</strong> ${composition.id}</p>`;
                    html += `<p><strong>创建时间:</strong> ${new Date(composition.created_at).toLocaleString()}</p>`;
                    ...
                    if (composition.validation_results) {
                        ...
                        if (vr.errors && vr.errors.length > 0) {
                            ...
                            vr.errors.forEach(err => html += `<li>${err}</li>`);
                            html += `</ul>`;
                        }
                    }
                    ...
                    if (composition.composition_data) {
                        ...
                        if (cd.services && cd.services.length > 0) {
                            ...
                            cd.services.forEach(service => {
                                html += `<li>
                                    <strong>${service.service_name} (ID: ${service.service_id})</strong>
                                    <p>角色: ${service.role}</p>
                                    <p>优先级: ${'🔥'.repeat(parseInt(service.priority) || 1)} (${service.priority})</p>
                                    <p>输入: ${service.inputs.join(', ') || '无'}</p>
                                    <p>输出: ${service.outputs.join(', ') || '无'}</p>
                                </li>`;
                            });
                            html += `</ul>`;
                        }
                    ...
                    html += `</div>`;
                    compositionData.innerHTML = html;
